...
static struct cxd2878_config tbs5530_cxd2878_config;
                    ...
                    struct i2c_msg msg[], int num)
{
    ...
    case 2:
        ...
        tbs5530_op_rw(d->udev, 0x90, 0, 0, buf6, 3, tbs5530_WRITE_MSG);
        tbs5530_op_rw(d->udev, 0x91, 0, 0, inbuf, buf6[0], tbs5530_READ_MSG);
    ...
    case 1:
        ...
        tbs5530_op_rw(d->udev, 0x80, 0, 0, buf6, msg[0].len + 2, tbs5530_WRITE_MSG);
        break;
    }
...
static int tbs5530_read_mac_address(struct dvb_usb_device *d, u8 mac[6])
{
    ...
    for (i = 0; i < 256; i++) {
        ...
        ibuf[1] = 0xa0;
        ibuf[2] = i;
        ret = tbs5530_op_rw(d->udev, 0x90, 0, 0, ibuf, 3, tbs5530_WRITE_MSG);
        ret = tbs5530_op_rw(d->udev, 0x91, 0, 0, ibuf, 1, tbs5530_READ_MSG);
        if (ret < 0) {
...
static int tbs5530_sat_streaming_ctrl(struct dvb_usb_adapter *adap, int onoff)
{
    struct dvb_usb_device *d = adap->dev;
    u8 buf[20];
    buf[0] = 10;
    buf[1] = 1;
    tbs5530_op_rw(d->udev, 0x8a, 0, 0, buf, 2, tbs5530_WRITE_MSG);
    return 0;
}

static int tbs5530_ter_cable_streaming_ctrl(struct dvb_usb_adapter *adap, int onoff)
{
    struct dvb_usb_device *d = adap->dev;
    u8 buf[20];
    buf[0] = 10;
...
static int tbs5530_frontend_cxd2878_attach(struct dvb_usb_adapter *adap)
{
    ...
    /* attach frontend */
    adap->fe_adap[0].fe = dvb_attach(cxd2878_attach, &tbs5530_cxd2878_config, &d->i2c_adap);
...
static int tbs5530_frontend_m88rs6060_attach(struct dvb_usb_adapter *adap)
{
    ...
    buf[0] = 0;
    buf[1] = 0;
    tbs5530_op_rw(d->udev, 0xb7, 0, 0, buf, 2, tbs5530_WRITE_MSG);
    buf[0] = 8;
    buf[1] = 1;
    tbs5530_op_rw(d->udev, 0x8a, 0, 0, buf, 2, tbs5530_WRITE_MSG);
    msleep(10);
