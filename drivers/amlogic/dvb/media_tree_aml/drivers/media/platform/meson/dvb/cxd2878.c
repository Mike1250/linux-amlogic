...
/* Added by automatic 4.9 compatibility pass: required headers */
...
err:
	...
	pr_err("set bank and registerbits error.\n");
...
err:
	...
	pr_err("%s %s: set bank and registerbits error.\n", 

       dev_driver_string(&dev->base->i2c->dev),

       dev_name(&dev->base->i2c->dev));
...
err:
	...
	pr_err("%s %s: cxd2878_atscSlaveRWriteRegister error.\n",
...
err:
	...
	pr_err("%s %s: slaveRWriteMultiRegisters error.\n",
...
err:
	...
	pr_err("%s %s %s: cxd2878_atsc_softreset error!\n", 
...
err:
	...
	pr_err("%s %s %s: %sable the repeater failed!\n",
...
err:
	...
	pr_err("%s %s %s: ascot3_read_rssi failed!\n",
...

{
    ...
    pr_info("Detected the Tuner chip ASCOT3, ID 2878A\n");
...
err:
	...
	pr_err("%s %s %s: Tuner ASCOT3 I2C error!\n",
...

{
	...
	err:
		...
		pr_err("%s %s %s: cxd2878_setstreamoutput error!\n",
...
err:
	...
	pr_err("%s %s %s: set TSClkModeAndFreq error!\n",
...
err:
	...
	pr_err("%s %s %s: cxd2878_setTSDataPinHiZ error!\n",
...
err:
	...
	pr_err("%s %s %s: SLtoAT_BandSetting error!\n",
...
err:
	...
	pr_err("%s %s %s: SLtoAT2_BandSetting error!\n",
...
err:
	...
	pr_err("%s %s %s: SLtoAC band setting error!\n",
...
err:
	...
	pr_err("%s %s %s: SLtoAC conversion error!\n",
...
err:
	...
	pr_err("%s %s %s: DVB-C tuning error!\n",
...
err:
	...
	pr_err("%s %s %s: SLtoAIT band setting error!\n",
...
err:
	...
	pr_err("%s %s %s: SLtoAIT conversion error!\n",
...
err:
	...
	pr_err("%s %s %s: ISDB-T tuning error!\n",
...
err:
	...
	pr_err("%s %s %s: SLtoACC band setting error!\n",
...
err:
	...
	pr_err("%s %s %s: SLtoACC frequency conversion error!\n",
...
err:
	...
	pr_err("%s %s %s: MCNS configuration error!\n",
...
err:
	...
	pr_err("%s %s %s: SLtoAA frequency conversion error!\n",
...
err:
	...
	pr_err("%s %s %s: ATSC tuning error!\n",
...
pr_err("%s %s %s: Initialization failed!\n",
...
pr_err("%s %s %s: Failed to read status!\n",
...
pr_err("%s %s %s: Frontend configuration failed!\n",
...
/* Added minimal definition for struct ecp3_info for kernel-4.9 build */
struct ecp3_info {
	unsigned int reg;
	void *data;
};

// Remove or fix the unused functions
#if 0
static void cxd2878_spi_read(struct dvb_frontend *fe, struct ecp3_info *ecp3inf)
{
    ...
    if (dev->base->config->read_properties) {
        u32 data;
        dev->base->config->read_properties(dev->base->i2c, ecp3inf->reg, &data);
        ecp3inf->data = (void*)(uintptr_t)data;
    }
}
...
static void cxd2878_spi_write(struct dvb_frontend *fe, struct ecp3_info *ecp3inf)
{
    ...
    if (dev->base->config->write_properties) {
        u32 data = (u32)(uintptr_t)ecp3inf->data;
        dev->base->config->write_properties(dev->base->i2c, ecp3inf->reg, data);
    }
}
...

{
    ...
    case SONY_DEMOD_CHIP_ID_CXD2856:  /**< CXD2856 / CXD6800(SiP) */
        ...
        pr_info("Detect CXD2856/CXD6800(SiP) chip.\n");
    ...
    case SONY_DEMOD_CHIP_ID_CXD2878:  /**< CXD2878 / CXD6801(SiP) */
        ...
        pr_info("Detect CXD2878/CXD6801(SiP) chip.\n");
    ...
    case SONY_DEMOD_CHIP_ID_CXD6802:  /**< CXD6802(SiP) */
        ...
        pr_info("Detect CXD2878/CXD6802(SiP) chip.\n");
    ...
    case SONY_DEMOD_CHIP_ID_UNKNOWN:  /**< Unknown */
        ...
        pr_err("%s: Cannot detect the chip.\n", KBUILD_MODNAME);
	...
	pr_debug("%s: attaching frontend successfully.\n", KBUILD_MODNAME);
...
err:
	...
	pr_err("%s %s %s: Failed to attach frontend!\n",
