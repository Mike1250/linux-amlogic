...
/* Compatibility defines for older kernels: */
#ifndef APSK_64
#define APSK_64 APSK_32
#endif
#ifndef FEC_1_4
#define FEC_1_4 FEC_1_2
#endif
#ifndef FEC_1_3
#define FEC_1_3 FEC_2_3
#endif
#ifndef FEC_5_9
#define FEC_5_9 FEC_8_9
#endif
#ifndef FEC_7_9
#define FEC_7_9 FEC_8_9
#endif
#ifndef FEC_4_15
#define FEC_4_15 FEC_4_5
#endif
#ifndef FEC_7_15
#define FEC_7_15 FEC_2_5
#endif
#ifndef FEC_8_15
#define FEC_8_15 FEC_2_5
#endif
#ifndef FEC_11_15
#define FEC_11_15 FEC_2_5
#endif
#ifndef FEC_13_18
#define FEC_13_18 FEC_9_10
#endif
#ifndef FEC_9_20
#define FEC_9_20 FEC_9_10
#endif
#ifndef FEC_11_20
#define FEC_11_20 FEC_1_2
#endif
#ifndef FEC_23_36
#define FEC_23_36 FEC_2_3
#endif
#ifndef FEC_25_36
#define FEC_25_36 FEC_5_6
#endif
#ifndef FEC_11_45
#define FEC_11_45 FEC_2_5
#endif
#ifndef FEC_13_45
#define FEC_13_45 FEC_3_5
#endif
#ifndef FEC_14_45
#define FEC_14_45 FEC_4_5
#endif
#ifndef FEC_26_45
#define FEC_26_45 FEC_2_5
#endif
#ifndef FEC_28_45
#define FEC_28_45 FEC_2_5
#endif
#ifndef FEC_29_45
#define FEC_29_45 FEC_2_5
#endif
#ifndef FEC_31_45
#define FEC_31_45 FEC_3_5
#endif
#ifndef FEC_32_45
#define FEC_32_45 FEC_2_5
#endif
#ifndef FEC_77_90
#define FEC_77_90 FEC_9_10
#endif
#ifndef ROLLOFF_15
#define ROLLOFF_15 ROLLOFF_25
#endif
#ifndef ROLLOFF_10
#define ROLLOFF_10 ROLLOFF_20
#endif
#ifndef ROLLOFF_5
#define ROLLOFF_5 ROLLOFF_25
#endif


/* --- Kernel 4.9 compatibility shim ---
 * This block provides fallbacks and aliases so the driver can compile against
 * older kernels (Linux 4.9) while keeping the newer source unchanged where
 * possible.  It's intentionally conservative — if your build still errors,
 * please paste the first build errors and I'll fix them quickly.
 */
...
/* Some kernels changed the DVB API names for frontend ops; provide weak
 * aliases if newer names are not available.  If build errors reference
 * specific symbols, I'll add targeted aliases.
 */
...

{
	...
	if (ret != 2) {
		pr_err("si5351 (ret=%d, reg=0x%02x)\n",
			 ret, reg);
		return -EREMOTEIO;
	}

	pr_debug("si5351 reg 0x%02x, value 0x%02x\n",
		 reg, *data);
...
static int rs6060_set_reg(struct m88rs6060_dev *dev, u8 reg, u8 data)
{
	...
	if (ret != 1) {
	 ...
	 pr_debug("0x%02x reg 0x%02x, value 0x%02x\n",
...
static int rs6060_get_reg(struct m88rs6060_dev *dev, u8 reg)
{
	...
	ret = i2c_transfer(i2c, msg, 2);
	if (ret != 2) {
		pr_err("0x%02x (ret=%d, reg=0x%02x)\n",
...
static int m88rs6060_firmware_download(struct m88rs6060_dev *dev, u8 reg,
...
static void m88rs6060_hard_reset(struct m88rs6060_dev *dev)
{
	unsigned val;
...
static void m88rs6060_set_ts_mode(struct m88rs6060_dev *dev)
{
	unsigned tmp, val = 0;
	regmap_read(dev->regmap, 0x0b, &val);
	val &= ~0x1;
...
static int m88rs6060_set_frontend(struct dvb_frontend *fe)
{
	...
	bool mis = false;
	 ...
	 pr_debug("delivery_system=%u modulation=%u frequency=%u bandwidth_hz=%u symbol_rate=%u inversion=%u stream_id=%d\n",
	...
	if(p_info.type==MtFeType_DvbS2 || p_info.type == MtFeType_DvbS2X){
		...
		if(dev->config.ts_mode==MtFeTsOutMode_Serial){
			...
			if(input_datarate != 0)
				...
				divid_ratio = 0xff;
			pr_debug("MClk_KHz = %d,divid_ratio = %d \n", Mclk_KHz, divid_ratio);
				 ...
				 enum fe_status *status)
{
	...
	*status = 0;
	...
	case SYS_DVBS2:
		...
		break;
	default:
    pr_debug("%s: invalid delivery_system\n", dev_name(&i2c->dev));
    ret = -EINVAL;
    goto err;
	}

	dev->fe_status = *status;
	pr_debug("%s: lock=%02x status=%02x\n", dev_name(&i2c->dev), utmp, *status);
...
#define M88rs6060_SNR_ITERATIONS 10
	...
	if (dev->fe_status & FE_HAS_LOCK) {
		...
		case SYS_DVBS2:
			...
			break;
		default:
    pr_debug("invalid delivery_system\n");
    ret = -EINVAL;
    goto err;
}
			      ...
			      enum fe_sec_tone_mode fe_sec_tone_mode)
{
	...
	case SEC_TONE_OFF:
		...
		break;
	default:
		pr_debug("invalid fe_sec_tone_mode\n");
		ret = -EINVAL;
		goto err;
	}
...
#define SEND_MASTER_CMD_TIMEOUT 120
	...
	if (utmp == 0) {
    pr_debug("%s: diseqc tx took %u ms\n", dev_name(&i2c->dev),
            jiffies_to_msecs(jiffies) -
            (jiffies_to_msecs(timeout) - SEND_MASTER_CMD_TIMEOUT));
} else {
    pr_debug("%s: diseqc tx timeout\n", dev_name(&i2c->dev));
				       ...
				       enum fe_sec_mini_cmd fe_sec_mini_cmd)
{
	...
	pr_debug("%s %s: fe_sec_mini_cmd=%d\n",
        dev_driver_string(&i2c->dev),
        dev_name(&i2c->dev),
        fe_sec_mini_cmd);
	...
	case SEC_MINI_B:
		...
		break;
	default:
		pr_debug("%s %s: invalid fe_sec_mini_cmd\n",
        dev_driver_string(&i2c->dev),
        dev_name(&i2c->dev));
ret = -EINVAL;
		goto err;
...
/* DISABLED for 4.9 build: static void m88rs6060_spi_read(struct dvb_frontend *fe,
 ...
 *//* DISABLED for 4.9 build: static void m88rs6060_spi_write(struct dvb_frontend *fe,
				struct ecp3_info *ecp3inf)
{
		struct i2c_client *client = fe->demodulator_priv;
	struct m88rs6060_dev *dev = i2c_get_clientdata(client);
 ...
 *//* DISABLED for 4.9 build: static void m88rs6060_eeprom_read(struct dvb_frontend *fe, struct eeprom_info *eepinf)
{
	struct i2c_client *client = fe->demodulator_priv;
	struct m88rs6060_dev *dev = i2c_get_clientdata(client);
	struct i2c_adapter *i2c = dev->base->i2c;
 ...
 *//* DISABLED for 4.9 build: static void m88rs6060_eeprom_write(struct dvb_frontend *fe,struct eeprom_info *eepinf)
{
	struct i2c_client *client = fe->demodulator_priv;
	struct m88rs6060_dev *dev = i2c_get_clientdata(client);
	struct i2c_adapter *i2c = dev->base->i2c;
 ...
 */static const struct dvb_frontend_ops m88rs6060_ops = {
	...
	.diseqc_send_master_cmd = m88rs6060_diseqc_send_master_cmd,/* .spi_read omitted for 4.9 *//* .spi_write omitted for 4.9 *//* .eeprom_read omitted for 4.9 *//* .eeprom_write omitted for 4.9 */};
static int m88rs6060_ready(struct m88rs6060_dev *dev)
{
    ...
    pr_info("found a '%s' in cold state\n", dev->fe.ops.info.name);
    ...
    pr_info("downloading firmware from file '%s'\n", name);

    ret = regmap_write(dev->regmap, 0xb2, 0x01);
    if (ret)
        goto err_release_firmware;

    pr_debug("firmware size = %lu data %02x %02x %02x\n",
    ...
    for (rem = firmware->size; rem > 0; rem -= (dev->config.i2c_wr_max - 1)) {
        ...
        ret = m88rs6060_firmware_download(dev, 0xb0,
                          &firmware->data[firmware->size - rem],
                          len);
        if (ret) {
            pr_err("firmware download failed len %d %d\n", len, ret);
            goto err_release_firmware;
        }
    }
    ...
    if (!val) {
        ret = -EINVAL;
        pr_info("firmware did not run\n");
        goto err;
    }

    pr_info("found a '%s' in warm state\n", dev->fe.ops.info.name);
    pr_info("firmware version:%X\n", val);
    msleep(5);
    m88rs6060_set_ts_mode(dev);
...
static int m88rs6060_probe(struct i2c_client *client, const struct i2c_device_id *id) {
    (void)id; /* compat */
	...
	*cfg->fe = &dev->fe;
	...
	pr_debug("%s: found the chip of %s\n", 
        dev_name(&client->dev), 
...
m88rs6060_ready(dev);
 ...
 err_kfree:
	...
	pr_warn("m88rs6060: probe failed = %d\n", ret);
	return ret;
}
